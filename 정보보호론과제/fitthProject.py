...
if __name__ == '__main__':
    '''
    Plaintext2 실행시 Plaintext1 부분을 전부 Plaintext2로 바꿔주기 
    '''
    # 선언
    IV_encCBC = int(str(1010101), 2)  # IV
    IV_decCBC = int(str(1010101), 2)
    IV_encOFB = int(str(1010101), 2)
    IV_decOFB = int(str(1010101), 2)
    Key = int(str(101010101), 2) #key
    Plaintext1 = "happy poppy"
    Plaintext2 = "chaeyoungju"
    # ECB
    encECB = []
    decECB = []
    for i in Plaintext1:
        encECB.append(encrypt(Key, ord(i)))
    for i in encECB:
        decECB.append(chr(decrypt(Key, i)))
    print(encECB, decECB)
    # CBC
    encCBC = []
    decCBC = []
    for i in Plaintext1:
        IV_encCBC = IV_encCBC ^ ord(i)
        insert_IV = encrypt(Key, IV_encCBC)
        encCBC.append(insert_IV)
        IV_encCBC = insert_IV
    for i in encCBC:
        IV_decCBC = IV_decCBC ^ decrypt(Key, i)
        decCBC.append(chr(IV_decCBC))
        IV_decCBC = i
    print(encCBC, decCBC)
    # OFB
    encOFB = []
    decOFB = []
    for i in Plaintext1:
        dump = encrypt(Key, IV_encOFB)
        encOFB.append(ord(i) ^ dump)
        IV_encOFB = dump
    for i in encOFB:
        dump = encrypt(Key, IV_decOFB)
        decOFB.append(chr(i ^ dump))
        IV_decOFB = dump
    print(encOFB, decOFB)
